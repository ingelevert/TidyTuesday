...
# site analysis, top 10 worst =


def analyze_sites(df):
    # Calculate average enterococci by site
    site_avg = df.groupby('swim_site')['enterococci_cfu_100ml'].agg(['mean', 'median', 'count']).reset_index()
    site_avg = site_avg[site_avg['count'] > 3]  # Filter for sites with enough measurements
    
    # Sort by median (more robust to outliers than mean)
    worst_sites = site_avg.sort_values(by='median', ascending=False).head(10)
    best_sites = site_avg.sort_values(by='median').head(10)
    
    # Plot worst sites
    plt.figure(figsize=(12, 8))
    bars = plt.barh(worst_sites['swim_site'][::-1], worst_sites['median'][::-1], color='#d73027')
    plt.title('Top 10 Beaches with Highest Enterococci Levels')
    plt.xlabel('Median Enterococci (CFU/100ml)')
    ...
    for bar in bars:
        ...
        plt.text(
            width + 5, 
            y, 
            f"{width:.0f}",  # Use the actual bar width (median value)
            va='center'
        )
    
    plt.tight_layout()
    plt.savefig("2025/Week_20_Water_Quality_Sydney/Graphs/worst_sites_enterococci.png", dpi=300)
    plt.close()

    return site_avg

# Call the site analysis function
site_results = analyze_sites(Water_Quality_clean)
print("Site analysis completed and visualizations saved.")

# Trying to create interactive map

def create_beach_map(df):
    # Calculate median enterococci for each site
    site_data = df.groupby(['swim_site', 'latitude', 'longitude'])['enterococci_cfu_100ml'].median().reset_index()
    
    # Create map centered on Sydney
    sydney_map = folium.Map(
        location=[-33.85, 151.20], 
        zoom_start=11,
    ...
    # Define color function based on water quality category
    def get_color(value):
        if pd.isna(value):
            return 'gray'
        elif value <= 40:
            ...
            return 'red'
    # Add markers for each swimming site
    for _, site in site_data.iterrows():
        enterococci = site['enterococci_cfu_100ml']
        color = get_color(enterococci)
        
        # Create popup content
        popup_content = f"""
        <div style="font-family: Arial; width: 200px;">
            <h4>{site['swim_site']}</h4>
            <b>Median Enterococci:</b> {enterococci:.1f} CFU/100ml<br>
            <b>Status:</b> <span style="color:{color};">
                {
                    'Good (≤40 CFU)' if enterococci <= 40 else
                    'Fair (41-200 CFU)' if enterococci <= 200 else
                    'Poor (>200 CFU)'
                }
            </span>
        </div>
    ...
    <div style="position: fixed; bottom: 50px; left: 50px; z-index:1000; padding: 10px;
        ...
        <h4>Water Quality</h4>
        <div><i class="fa fa-circle" style="color:green;"></i> Good (≤40 CFU/100ml)</div>
        <div><i class="fa fa-circle" style="color:orange;"></i> Fair (41-200 CFU/100ml)</div>
        <div><i class="fa fa-circle" style="color:red;"></i> Poor (>200 CFU/100ml)</div>
    </div>
    '''
    sydney_map.get_root().html.add_child(folium.Element(legend_html))
    
    # Save map
    sydney_map.save("2025/Week_20_Water_Quality_Sydney/Graphs/sydney_beaches_map.html")
